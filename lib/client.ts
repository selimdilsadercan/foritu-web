...
export function Environment(name: string): BaseURL {
  return `https://${name}-foritu-h2ai.encr.app`;
}
...
/**
 * Client is an API client for the foritu-h2ai Encore application.
 */
...
export namespace transcript {
  ...
  export class ServiceClient {
    ...
    public async DeleteTranscript(
      ...
      const resp = await this.baseClient.callTypedAPI(
        "DELETE",
        `/transcript/${encodeURIComponent(userID)}`
      );
    ...
    public async ParseAndStoreTranscript(
      params: ParseAndStoreTranscriptRequest
    ): Promise<ParseAndStoreTranscriptResponse> {
      // Now make the actual call to the API
      const resp = await this.baseClient.callTypedAPI(
        "POST",
    ...
    public async StoreTranscript(
      params: StoreTranscriptRequest
    ): Promise<StoreTranscriptResponse> {
      // Now make the actual call to the API
      const resp = await this.baseClient.callTypedAPI(
        "POST",
        `/transcript`,
    ...
    public async UpdateTranscript(
      userID: string,
      params: UpdateTranscriptRequest
  ...
  constructor(baseURL: string, options: ClientOptions) {
    ...
    if (!BROWSER) {
      this.headers["User-Agent"] =
        "foritu-h2ai-Generated-TS-Client (Encore/v1.48.12)";
    }
